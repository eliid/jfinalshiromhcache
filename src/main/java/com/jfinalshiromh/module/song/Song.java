...
public class Song extends BaseModel<Song> {
	...
	public List<Song> getNewSong(Integer pageSize,Integer pageNum){
		return me.find("select id,name,author,level,cover_path,stave_path,midi_path,ogg_path,click_num from lkk_song  order by id desc limit ?,?",(pageNum-1)*pageSize,pageSize);
	}
	
	public List<Song> getHotSong(Integer pageNum,Integer pageSize){
		Long count = Db.queryFirst("select count(*) from lkk_song where click_num>=100");
		if(count>=3){
			return me.find("select id,name,author,level,cover_path,stave_path,midi_path,ogg_path,click_num from lkk_song  order by click_num desc limit ?,?",(pageNum-1)*pageSize,pageSize);
		}else{
			return null;
		}
	}
	...
	public List<Map<String,Object>> getListSongByTagId(Integer tagId,Integer pageNum,Integer pageSize){
		...
		for(int i=1;i<=8;i++){
			List<Song> songs = me.find("select DISTINCT(st.song_id) id,name,author,level,cover_path,stave_path,midi_path,ogg_path,"
					+ "click_num from lkk_song s,lkk_song_tag st where s.id=st.song_id and st.tag_id=? and level=? order by s.id desc limit ?,?",tagId,i,(pageNum-1)*pageSize,pageSize);
	...
	public List<Song> getListSongByTagIdAndLevel(Integer tagId,Integer level,Integer pageNum,Integer pageSize){
		return  me.find("select DISTINCT(st.song_id) id,name,author,level,cover_path,stave_path,midi_path,ogg_path,"
				+ "click_num from lkk_song s,lkk_song_tag st where s.id=st.song_id and st.tag_id=? and level=? order by s.id desc limit ?,?",tagId,level,(pageNum-1)*pageSize,pageSize);
	}
