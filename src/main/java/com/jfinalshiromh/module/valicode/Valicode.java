...
public class Valicode extends BaseModel<Valicode> implements Serializable {
    ...
    public Valicode findByCodeAndEmail(String code, String email, String type) {
        ...
        return super.findFirst("select * from lkk_valicode v where v.status = 0 and v.code = ? and v.target = ? and v.expire_time > ? and v.type = ?", code, email, nowTime, type);
    }

    
    public Valicode findByCodeAndPhone(String code, String phone, String type) {
        String nowTime = DateUtil.formatDateTime(new Date());
        return super.findFirst("select * from lkk_valicode v where v.status = 0 and v.code = ? and v.target = ? and v.expire_time > ? and v.type = ?", code, phone, nowTime, type);
    }
}
