...
public class ParentController extends BaseController {
	...
	public void changeAddrHtml(){
		...
		Student student = Student.me.findFirst("select address from lkk_student where phone = ? ", user.getStr("phone"));
	...
	public void changeAddr(){
		...
		if(Db.update("update lkk_student set address=? where phone=?", addr,user.getStr("phone"))>0){
			success();
		}else{
			 error("修改地址失败");
		}
	...
	public void bindPhone(){
		...
		Db.update("update lkk_user set phone=? where phone=?", newPhone,phone);
		Db.update("update lkk_student set phone=? where phone=?", newPhone,phone);
		removeSessionAttr(Constants.USER_SESSION);
		removeCookie(Constants.USER_COOKIE);
		success();
