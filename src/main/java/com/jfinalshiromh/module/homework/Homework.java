...
@TableBind(tableName = "lkk_homework")
public class Homework extends BaseModel<Homework> {
    public static String TABLE_NAME = "lkk_homework";
    ...
    public Page<Homework> page(int pageNumber, int pageSize, String name) {
        ...
        return super.paginate(pageNumber, pageSize, "select h.*, c.name cname,p.name pname ",
                "from " + TABLE_NAME + " h,lkk_course_part p,lkk_course c " +
    ...
    public List<Homework> getAllByPartId(Integer pid){
    	return find("select * from lkk_homework where course_part_id in ("+
				"select id from lkk_course_part where parent_id in("+
				"select id from lkk_course_part where parent_id=? order by part_index) and type=5) order by source_index",pid);
    }
