...
public class Greetings extends BaseModel<Greetings> {
    ...
    public List<Greetings> getAllByClientTypeAndType(String clientType,String type){
        return find("select content from lkk_greetings where client_type=? and type=?",clientType,type);
    }

    public List<String> getTypesByClientType(String clientType){
        return Db.query("select type from lkk_greetings where client_type=? group by type",clientType);
    }
    public List<Object> getClientTypeById(Integer id){
        return Db.query("select clientType from lkk_greetings where id=? group by clientType",id);
    }

    public List<Greetings> getAllByClientType(String clientType){
        return find("select content,type from lkk_greetings where client_type=?",clientType);
    }
    public String getTypeById(Integer id){
        return Db.queryStr("select clientType from lkk_greetings where id=?",id);
    }
