...
public class Teacher extends BaseModel<Teacher> {
    ...
    private String TABLE_NAME = "lkk_teacher";
    ...
    public Page<Teacher> page(int pageNumber, int pageSize, String name) {
        ...
        return super.paginate(pageNumber, pageSize, "select * ",
                "from lkk_teacher where 1 = 1 " + condition + " order by id desc");
    }
    ...
    public Page<Teacher> pageBySchool(int pageNumber, int pageSize, String name,Integer schoolId) {
        ...
        return super.paginate(pageNumber, pageSize, "select t.id,t.name,t.job,u.avatar,t.phone ",
                "from lkk_teacher t,lkk_user u where u.phone=t.phone and t.school_id="+schoolId+" " + condition + " order by id desc");
    }
    
    public Teacher getByTid(Integer tid){
    	return me.findFirst("select t.*,u.avatar from lkk_teacher t,lkk_user u where u.phone=t.phone and t.id=?",tid);
    }
    ...
    public Page<Teacher> pageByCid(int pageNumber, int pageSize, int classId) {
       return super.paginate(pageNumber,pageSize,"select t.id,t.name,u.avatar,t.job",
               "from lkk_teacher t, lkk_class_teacher ct ,lkk_user u where t.id = ct.tid and u.phone=t.phone and ct.cid = ?",classId);
    }
    
    
    public List<Teacher> getByCid(int classId) {
       return me.find("select t.id,t.name,u.avatar,t.job from lkk_teacher t, lkk_class_teacher ct ,lkk_user u where t.id = ct.tid and u.phone=t.phone and ct.cid = ?",classId);
    }
    ...
    public Page<Teacher> pageByNotCid(int pageNumber, int pageSize,Integer schoolId,Integer classId) {
        ...
        return super.paginate(pageNumber,pageSize,"select t.id,t.name,t.job,u.avatar,t.phone ",
                "from "+ TABLE_NAME +" t, lkk_user u where u.phone=t.phone and t.school_id =? and t.id not in (select tid from lkk_class_teacher where cid =?)",schoolId,classId);
    }

    public List<Teacher> list() {
        return super.find("select * from lkk_teacher");
    }

    public long getAllTeacherNumByClass(int classId) {
        return Db.queryLong("select count(*) from lkk_class_teacher where cid = ? ", classId);
    }
