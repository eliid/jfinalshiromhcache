...
public class StudentScore extends BaseModel<StudentScore> {
    ...
    public Page<StudentScore> page(int pageNumber, int pageSize, String name) {
        ...
        return super.paginate(pageNumber, pageSize, "select ss.id, " +
                        ...
                        "ss.s_shizhi," +
                        "ss.student_answer",
                "from " + TABLE_NAME + " ss,lkk_student stu,lkk_course_part cp where 1=1 " +
                        condition +
    ...
    public StudentScore getByPhoneAndPId(Integer sid,Integer pid){
    	return findFirst("select s.*,p.name pname from lkk_student_score s,lkk_course_part p where p.id=s.part_id and s.sid=? and part_id=?",sid,pid);
    }
    
    public StudentScore getZuoYeByPartIdAndPhone(Integer sid,Integer pid){
    	return findFirst("select s.* from lkk_course_part c,lkk_student_score s where c.id=s.part_id and c.parent_id=? and c.type=107 and s.sid=?",pid,sid);
    }
    
    public List<StudentScore> getAllLessonByStu(Integer courseId,Integer currentLessonId,Integer sid){
    	return find("select s.*,c.name name from lkk_course_part c ,lkk_student_score s where c.id=s.part_id and c.course_id=? and c.parent_id=0 and c.id<=? and s.sid=? order by part_index desc",courseId,currentLessonId,sid);
    }
    
    public List<StudentScore> getAllLessonUnitByStuAndLessonId(Integer parentPId,Integer courseId,Integer sid){
    	return find("select * from lkk_course_part c ,lkk_student_score s where c.id=s.part_id and c.course_id=? and c.parent_id=? and s.sid=? order by part_index",courseId,parentPId,sid);
    }
    
    
    public List<StudentScore> getAllTaskByStu(Integer courseId,Integer currentLessonId,Integer sid){
    	return find("select s.*,c.parent_id parentId  from lkk_course_part c,lkk_student_score s where s.part_id=c.id and parent_id in(select id from lkk_course_part "+
    ...
    public List<StudentScore> getTasksByLessonId(Integer pid,Integer sid){
    	return find("select s.*, c.name from lkk_course_part c,lkk_student_score s where s.part_id=c.id and c.parent_id=? and s.sid=? order by part_id",pid,sid);
    }

    public List<StudentScore> getStuScoreByClassAndPartId(Integer classId,Integer partId){
		return me.find("select s.name,ss.score,s.phone,s.id,ss.play_record,ss.error_flag,ss.comments from lkk_student s,lkk_student_score ss where s.id=ss.sid and s.class_id=? and ss.part_id=? order by ss.score desc",classId,partId);
	}
