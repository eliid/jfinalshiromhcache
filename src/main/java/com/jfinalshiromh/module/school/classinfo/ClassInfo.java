...
public class ClassInfo extends BaseModel<ClassInfo> {
    ...
    public Page<ClassInfo> pageBySchool(int pageNumber, int pageSize, String name,int gradeId,int classId,Integer schoolId) {
        ...
        return super.paginate(pageNumber, pageSize, "select c.id cid,c.name cname,lesson_id,g.id gid,g.name gname, g.course_id,co.part_number ", " from " + TABLE_NAME + " c left join lkk_grade g on g.id=c.gid left join lkk_course co on co.id=g.course_id where g.school_id="+schoolId + condition + " order by g.id, c.id");
    ...
    public String getGradeNameById(int cid){
        String sql = "select g.name from lkk_grade g,lkk_class c where 1=1 and g.id = c.gid and c.id = ?";
        return Db.queryStr(sql, cid);
    }
    
    public Object [] getGradeAndClassNameById(int cid){
       String sql = "select g.name gname,c.name cname,g.course_id course_id from lkk_grade g,lkk_class c where 1=1 and g.id = c.gid and c.id = ?";
       return Db.queryFirst(sql,cid);
    }
    
    public ClassInfo getByStuId(Integer stuId){
    	return findFirst("select c.* from "+TABLE_NAME+" c,lkk_class_student cs where c.id=cs.cid and sid=?",stuId);
    }
