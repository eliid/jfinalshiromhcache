...
public class ClassTeacher extends BaseModel<ClassTeacher> {
    ...
    public List<ClassTeacher> getClassGradeInfoByTid(Integer tid){
    	return me.find("select ct.id,c.id cid,c.name cname,g.id gid,g.name gname from lkk_class_teacher ct,lkk_class c,lkk_grade g where ct.cid=c.id and c.gid=g.id and ct.tid=? order by g.id,c.id",tid);
    }
