...
@TableBind(tableName = "lkk_course_part_history")
public class CoursepartHistory extends BaseModel<CoursepartHistory> {
    ...
    public static String TABLE_NAME = "lkk_course_part_history";
    ...
    public List<CoursepartHistory> getHistroyLessonByClass(Integer classId){
    	return find("select h.part_id partId,h.id, c.parent_id lessonId,name,type from lkk_course_part_history h, lkk_course_part c where h.part_id=c.id and h.class_id = ?",classId);
    }
