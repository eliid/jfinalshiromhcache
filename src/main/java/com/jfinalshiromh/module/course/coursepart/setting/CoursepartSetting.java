...
import com.jfinalshiromh.common.jfinalshiromhConfig;
import com.jfinalshiromh.utils.ext.plugin.tablebind.TableBind;

@TableBind(tableName = "lkk_course_part_setting")
public class CoursepartSetting extends BaseModel<CoursepartSetting> {
    ...
    public static String TABLE_NAME = "lkk_course_part_setting";
    ...
    public List<CoursepartSetting> getAllLesson(Integer classId){
    	return me.find("select c.name,s.lesson_status,s.id,s.part_id lesson_id,s.class_id from lkk_course_part_setting s,lkk_course_part c where s.part_id=c.id and s.class_id=? and s.parent_part_id=0 order by s.id",classId);
    }
    
    
    public List<CoursepartSetting> getAllPartByLesson(Integer lessonId,Integer classId){
    	return me.find("select c.name,c.type,s.part_id lesson_id from lkk_course_part_setting s,lkk_course_part c where s.part_id=c.id and s.parent_part_id=? and s.class_id=? order by s.id",lessonId,classId);
    }
