...
public class Coursepart extends BaseModel<Coursepart> {
    ...
    public Page<Record> page(int pageNumber, int pageSize, String name, String courseName, int parentpartId, int courseId) {
        ...
        from lkk_course_part p,lkk_course_part pp,lkk_course c
        where 1 = 1
        ...
        */
        ...
        return Db.paginate(pageNumber, pageSize, "select p.id,p.name,p.part_index,p.unit_number,p.type," +
                        ...
                        "pp.name parentname",
                "from " + TABLE_NAME + " p,lkk_course_part pp,lkk_course c " +
    ...
    public Coursepart getPrevByIdAndGrade(Integer id,Integer courseId){
        return me.findFirst("select * from lkk_course_part where course_id=? and parent_id=0 and id<? order by part_index desc limit 1", courseId,id);
    }
    public Coursepart getNextByIdAndGrade(Integer id,Integer courseId){
        return me.findFirst("select * from lkk_course_part where course_id=? and parent_id=0 and id>? order by part_index limit 1", courseId,id);
    }
    
    public List<Coursepart> getChildsById(Integer id){
        return me.find("select * from lkk_course_part where parent_id=? order by part_index",id);
    }

    
    public Integer getTaskIdByLessonId(Integer id){
        return  Db.queryFirst("select id from lkk_course_part where parent_id=? and type=107",id);
    }
    
    public List<Coursepart> getLessonByGid(Integer gid){
        return find("select c.id,c.name from lkk_grade g,lkk_course_part c where g.course_id=c.course_id and g.id=? and c.parent_id=0 order by c.part_index",gid);
    }
    ...
    public List<Coursepart> getChildsColumnById(Integer id){
        return me.find("select id,name,type from lkk_course_part where parent_id=? order by part_index",id);
    }

    public Coursepart getById(Integer id){
        return findFirst("select id,name,type ,parent_id,part_index from lkk_course_part where id=?",id);
    }
