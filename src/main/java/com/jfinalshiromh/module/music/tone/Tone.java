...
public class Tone extends BaseModel<Tone> {
    ...
    public Page<Tone> page(int pageNumber, int pageSize, String name,String englishName,Integer parentId,String clientType) {
        ...
        return super.paginate(pageNumber, pageSize, "select a.id,a.name,a.english_name,b.name as parent_id,a.tone_id,a.client_type  ",
                "from lkk_tone as a,lkk_tone as b where a.parent_id=b.id" + condition + " order by a.id desc");
    }
    ...
    public List<Tone> getNameByParentIdAndType(Integer parent_id ,String client_type){
        return find("select name，english_name from lkk_tone where parent_id=? and client_type=?",parent_id,client_type);
    }

    public List<String> getNameByClientType(String clientType){
        return Db.query("select name,english_name from lkk_tone where client_type=? group by type",clientType);
    }
    public List<Object> getClientTypeById(Integer id){
        return Db.query("select client_type from lkk_tone where id=? group by client_type",id);
    }
    ...
    public  String getParentNameByParentId(Integer parentId){
        ...
        if (parentId!=null){
            return Db.queryStr( "select  name  from lkk_tone where id=?",parentId);
        }
        return null;
    ...
    public Integer getParentIdByToneId(Integer toneId){
        if (toneId!=null) {
            return Db.queryInt("select parent_id from lkk_tone where tone_id=?", toneId);
        } else{
    ...
    public String getToneNameByToneId(Integer toneId){
        if (toneId!=null) {
            return Db.queryStr("select name from lkk_tone where tone_id=?", toneId);
        } else{
            System.out.println("toneId==null!!!");
        }
        return null;
    ...
    public List<Tone> getByParentId(Integer Id){
            ...
            return find("select tone_id as ids ,name from lkk_tone where parent_id =? order by tone_id desc", Id);
