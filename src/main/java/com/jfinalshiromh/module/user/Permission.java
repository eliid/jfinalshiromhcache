...
public class Permission extends BaseModel<Permission> {
    ...
    public List<Permission> findByPid(Object pid) {
        return super.find("select * from lkk_permission where pid = ?", pid);
    }
    ...
    public Set<String> findPermissions(String username) {
        List<Permission> permissions = super.findByCache(Constants.SHIROCACHE, Constants.PERMISSIONCACHEKEY + username,
                "select p.* from lkk_admin_user u, lkk_role r, lkk_permission p, " +
                        "lkk_user_role ur, lkk_role_permission rp where u.id = ur.uid and r.id = ur.rid and r.id = rp.rid and p.id = rp.pid and u.username = ?", username);
