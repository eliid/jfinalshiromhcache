...
public class AdminUser extends BaseModel<AdminUser> {
    ...
    public Page<AdminUser> page(Integer pageNumber, Integer pageSize) {
        return super.paginate(pageNumber, pageSize, "select * ", "from lkk_admin_user order by in_time desc");
    }

    public AdminUser findByUsername(String username) {
        return super.findFirst("select * from lkk_admin_user where username = ?", username);
    }

    public void correlationRole(Integer userId, Integer[] roles) {
        
        Db.update("delete from lkk_user_role where uid = ?", userId);
