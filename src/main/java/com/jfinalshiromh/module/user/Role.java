...
public class Role extends BaseModel<Role> {
    ...
    public Page<Role> page(Integer pageNumber, Integer pageSize) {
        return super.paginate(pageNumber, pageSize, "select * ", "from lkk_role");
    }

    public List<Role> findAll() {
        return super.find("select * from lkk_role");
    }

    public Set<String> findRoles(String username) {
        List<Role> roles = super.findByCache(Constants.SHIROCACHE, Constants.ROLECACHEKEY + username,
                "select r.* from lkk_admin_user u, lkk_role r, lkk_user_role ur where u.id = ur.uid and r.id = ur.rid and u.username = ?", username);
    ...
    public void correlationPermission(Integer roleId, Integer[] permissionIds) {
        ...
        Db.update("delete from lkk_role_permission where rid = ?", roleId);
