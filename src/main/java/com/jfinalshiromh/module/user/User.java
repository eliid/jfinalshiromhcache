...
public class User extends BaseModel<User> {
    ...
    public User findByOpenID(String openId, String type) {
        ...
        if (type.equalsIgnoreCase(Constants.QQ)) {
            sql = "select u.* from lkk_user u where u.qq_open_id = ?";
        } else if (type.equalsIgnoreCase(Constants.SINA)) {
            sql = "select u.* from lkk_user u where u.sina_open_id = ?";
        }
        return super.findFirst(sql, openId);
    }

    public User findByToken(String token) {
        return super.findFirst("select u.* from lkk_user u where u.token = ?", token);
    }
    ...
    public User localLogin(String email, String password) {
        return super.findFirst("select * from lkk_user where email = ? and password = ?", email, password);
    }

    public User localPhoneLogin(String phone, String password, String type) {
        return super.findFirst("select u.*,t.id tid from lkk_user u ,lkk_teacher t where t.phone=u.phone and u.phone = ? and u.password = ? and u.type = ?", phone, password, type);
    }
    
    public User localStuPhoneLogin(String phone, String password, String type) {
        return super.findFirst("select u.*,g.name gname,g.id gid,c.id cid,c.name cname,s.id sid  from lkk_user u,lkk_student s,lkk_class c ,lkk_grade g "
    ...
    public User schoolPhoneLogin(String phone, String password) {
        return super.findFirst("select * from lkk_user where phone = ? and password = ? and type in ('4','2') ", phone, password);
    }


    public User findByEmail(String email) {
        return super.findFirst("select * from lkk_user where email = ?", email);
    }

    public User findByPhone(String phone) {
        return super.findFirst("select * from lkk_user where phone = ?", phone);
    }

    public User findByNickname(String nickname) {
        return super.findFirst("select * from lkk_user where nickname = ?", nickname);
    }
    ...
    public Integer findDayByAuthorId(String authorId) {
        ...
        return Db.queryInt("select max(day) from lkk_mission where author_id = ? and (in_time between ? and ?)", authorId, startTime, endTime);
    }

    public int countUsers() {
        return super.find("select id from lkk_user").size();
    }
    ...
    public Page<User> page(int pageNumber, int pageSize, String nickname, String email,String type,Integer schoolId) {
        ...
        return super.paginate(pageNumber, pageSize, "select * ",
                "from lkk_user where 1 = 1 " + condition + " order by in_time desc");
    }

    public List<User> list() {
        return super.find("select * from lkk_user");
    }
    ...
    public List<User> findToday() {
        ...
        return super.find("select nickname, email, qq_nickname, sina_nickname, qq_avatar, sina_avatar, in_time " +
                "from lkk_user where in_time between ? and ? order by in_time desc", start, end);
    }

    

    public int updatePhoneByPhone(String newPhone, String phone) {
        return Db.update("update lkk_user set phone=? where phone=?", newPhone, phone);
    }
    
    public int updatePicUrlByPhone(String picUrl,String phone){
    	return Db.update("update lkk_user set avatar=? where phone=?", picUrl, phone);
    }
    
    public int updateNameByPhone(String phone,String name){
    	return Db.update("update lkk_user set nickname=? where phone=?", name, phone);
    }
    
    
    public Boolean checkUserIsExist(String phone){
    	if(Db.queryLong("select count(*) from lkk_user where phone=?",phone)>0){
    		return false;
    	}
    	return true;
    }
    ...
    public int updatePassword(String phone,String password){
    	return Db.update("update lkk_user set password=? where phone=?",HashKit.md5(password),phone);

    }
    public int updatePasswordAndName(String phone,String password,String nickname){
    	return Db.update("update lkk_user set password=? ,nickname =? where phone=?",HashKit.md5(password),nickname,phone);
    }
    
    public int updateFirst(String phone){
    	return Db.update("update lkk_user set first_login=2 where phone=?",phone);
    }
